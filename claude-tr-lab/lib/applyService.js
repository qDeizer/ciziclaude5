...
function createApplyService({ logger, powershell, elevation, claudeProcess, lock, workRoot }) {
  ...
  async function grantWrite(absolutePath) {
  ...
  async function restoreProtection(absolutePath) {
    // Best-effort: eklenen ACE kaldirilir ve sahiplik TrustedInstaller'a doner.
    try {
      await powershell.run(
        "$f=[string]$env:CIZI_TARGET;"
        + `& icacls.exe $f /remove:g ${ADMINISTRATORS_SID} | Out-Null;`
        + `& icacls.exe $f /setowner '${TRUSTED_INSTALLER}' | Out-Null`,
        { env: { CIZI_TARGET: absolutePath }, timeoutMs: 60000 },
      );
      return true;
    } catch (cause) {
  ...
  async function applyLocked(packageInfo, provenance, stagedFiles) {
    ...
    try {
      ...
      for (const target of granted) await restoreProtection(target);
    ...
    for (const target of granted) protectionRestored.push(await restoreProtection(target));

    logger.success("apply", "Turkce yama uygulandi", {
      version: packageInfo.version,
      files: written.length,
      protectionRestored: protectionRestored.filter(Boolean).length,
    });
    return { applied: true, files: written, protectionRestored };
  }
  ...
  async function restore(packageInfo, { confirm = false } = {}) {
    ...
    try {
      ...
      for (const entry of manifest.files) {
        const livePath = path.join(packageInfo.installLocation, entry.relativePath.split("/").join(path.sep));
        await restoreProtection(livePath);
      }
      return result;
    } finally {
      held.release();
