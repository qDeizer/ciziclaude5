...
function createBuildService({
  ...
  function build(packageInfo, dictionary) {
    ...
    const scanResult = scanner.scan(packageInfo, dictionary.labels.rules);
    ...
    if (!stagedFiles.length) {
      // Iki cok farkli durum ayni sonucu verir: (a) sozluk bos/kurallar eslesmedi,
      ...
      const alreadyBranded = scanResult.labels.alreadyTranslated.length > 0
        || (brandTerms.length > 0 && scanResult.catalogs.some((catalog) => {
          const text = fs.readFileSync(catalog.absolutePath, "utf8");
          return brandTerms.some((term) => text.includes(term));
        }));
      if (alreadyBranded) {
        logger.warning("build", "Dosyalar zaten markalanmis ama uretim kaydi yok; yeniden uretilemez", {
          version: packageInfo.version,
          note: "Hash dogrulamasi yapilamaz; geri alma icin Claude onarimi/yeniden kurulumu gerekir.",
        });
        throw codedError(
          "ALREADY_BRANDED_WITHOUT_RECORD",
          "Claude dosyalari zaten markalanmis ancak Cizi Code'un uretim kaydi yok.",
        );
      }
    ...
    const provenance = {
      ...
      source: {
        ...
        installKind: packageInfo.installKind || "msix",
      },
      dictionary: {
